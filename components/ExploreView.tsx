...
export function ExploreView() {
    ...
    supabase
      ...
      .then(async ({ data }) => {
        ...
        if (user) {
          ...
          const likeRes = await supabase.from("likes").select("post_id, reaction_emoji").eq("user_id", user.id).in("post_id", postIds);
          for (const l of likeRes.data ?? []) { likedIds.add(l.post_id); reactionMap.set(l.post_id, l.reaction_emoji); }
        }
        setTrending(data.map((p) => ({ ...p, liked_by_me: likedIds.has(p.id), my_reaction: reactionMap.get(p.id) ?? null, pawmarked_by_me: false })));
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      setPosts(postData.map((p) => ({ ...p, liked_by_me: likedIds.has(p.id), my_reaction: reactionMap.get(p.id) ?? null, pawmarked_by_me: false })));
      setSearching(false);
    }, 300);
